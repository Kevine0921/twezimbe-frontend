      ...
      <div className="container mx-auto h-[100dvh] flex flex-col-reverse md:flex-row items-center justify-between space-y-8 md:space-y-0 gap-5">
        ...
        <div className="flex-1 text-center md:text-left space-y-6">
          ...
          <p className="text-lg text-gray-700">
            Providing financial assistance to help cover unexpected costs during
            difficult times.
          </p>
          <div className="flex justify-center md:justify-start space-x-4 w-full flex-col gap-2">
            ...
            <div className="p-3 mt-5">
              <h1>
                These are the groups you can create Fund for. Choose one to
                create BF for:{" "}
              </h1>
              ...
              {currentUser?.role === "Admin"
                ? groups?.length &&
                  groups.map((group, index) => {
                    ...
                    return (
                      <div
                        className={`p-2 mt-3 border-2 border-gray-800 rounded-md cursor-pointer flex items-center justify-between hover:bg-neutral-100 transition duration-75 ${
                          selectedGroup?._id === group?._id &&
                          "bg-orange-100 hover:bg-neutral-100"
                        }`}
                        onClick={() => {
                ...
                : joinedGroupList?.length &&
                  joinedGroupList?.map((group: GroupTypes, index: number) => {
                    if (
                      group?.created_by === currentUser?._id &&
                      !group?.has_bf
                    ) {
                      return (
                        <div
